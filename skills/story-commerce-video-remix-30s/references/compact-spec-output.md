...
- After showing the final `video_generation_prompt`, call `dynamic_questionnaire` once to ask whether to continue video generation or revise the plan. Do not rely on a plain-text consent sentence.
...
After this visible prompt, call `dynamic_questionnaire` with one radio question:
...
- 5-6 timestamped shots for 30 seconds
