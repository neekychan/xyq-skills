...
Use these rules whenever the output will be used by SD2.5 or another video generator.

成片稳定性优先级高于花哨剪辑。最终 `video_generation_prompt` 要让模型知道每一镜保持什么、何时切换、如何承接。

- Use one stable 5-6-shot structure for 30 seconds: `0-3s / 3-8s / 8-14s / 14-21s / 21-27s / 27-30s`.
...
- The final prompt should include a compact stability sentence:

```text
全片使用稳定5到6镜结构，每镜承载一个完整动作或证明点，每镜一个主要动作和一句短口播；商品、主体1、场景光线和手部动作保持连续，镜头在动作完成或口播结束后自然切换。
```
...
- Use this stable switch pattern for all categories: show the full set first -> pick up or point to one item -> use/show that item -> place it down or cover the lens -> pick up the next item -> continue / 所有品类都使用稳定切换模式：先集合展示 -> 拿起或指向一个商品 -> 使用/展示该商品 -> 放下或遮镜 -> 拿起下一个商品 -> 继续
- Acceptable transition devices: hand covers lens, object swipes across frame, product placed down then next product picked up, tabletop reset, rack/shelf/box layout cutaway, mirror reset, packaging close-up, creator points to the next item, or camera pans across a clearly arranged lineup / 可接受转场：手遮镜、物体扫镜、放下当前商品再拿起下一个、桌面重置、衣架/货架/盒内排列插入、镜前重置、包装特写、达人指向下一个、镜头扫过清晰排列的整组商品
...
- For clothing and try-on videos, never let one outfit morph into another on the body. Use a clear transition: hand covers lens, garment swipes across frame, mirror reset, dressing-room cut, rack/bed layout cutaway, or creator holds the next item to camera before the next try-on / 服装试穿视频里，绝不要让一套衣服在身上直接变成另一套。必须用明确转场：手遮镜、衣服扫过镜头、镜前重置、试衣间切换、衣架/床面展示插入、达人手持下一件靠近镜头后再试穿
...
- For stable video generation, use 5-6 connected shots for 30-second content by default / 面向稳定视频生成时，30 秒默认使用 5 到 6 个有承接的镜头
- Each shot should complete one action or proof point before switching; preferred 30s rhythm is `0-3s`, `3-8s`, `8-14s`, `14-21s`, `21-27s`, `27-30s` / 每个镜头完成一个动作或证明点再切换；30 秒优先节奏是 `0-3s`、`3-8s`、`8-14s`、`14-21s`、`21-27s`、`27-30s`
...
- Use fewer than 5 shots only when the user explicitly asks for a slower cinematic rhythm or when the product proof needs longer holds; every cut still follows a completed action / 只有用户明确要更慢的电影化节奏，或商品证明需要更长停留时才少于 5 镜；每次切换仍要接在动作完成后
...
- Bridge real scenes to screenshots or app pages through a visible device or gesture: creator lifts phone -> taps screen -> camera moves into phone screen -> page/screenshot fills frame -> camera returns to product or face / 实景到截图或 App 页面用设备或手势桥接：达人举起手机 -> 点击屏幕 -> 镜头推进到手机屏幕 -> 页面/截图铺满画面 -> 镜头回到商品或人脸
- Keep screenshots and UI pages grounded: show them on a phone, tablet, laptop, projected board, or picture-in-picture panel next to the creator/product / 截图和 UI 页面保持有载体：呈现在手机、平板、电脑、投影板，或作为人物/商品旁边的画中画面板
...
For 30-second effect-first commerce videos:
...
3-8s: product returns to main selling point, first proof action
8-14s: visible result, detail, comparison, process, or reaction
14-21s: second proof, usage context, or reaction
21-27s: value confirmation with product visible
27-30s: one CTA
```
...
- 3-8s must clearly return to the product and main selling point.
- 8-14s should be the most stable proof section, not a dense montage.
- 21-27s should confirm value while product remains visible.
- 27-30s should close with one CTA.
- Keep the whole video layered: fast enough to catch attention, stable enough for proof, calm enough for CTA.
- If using a meme, drama, absurd hook, or price-led opening, reconnect to product by 3-8s.
