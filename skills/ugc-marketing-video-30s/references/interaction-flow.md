...
- Keep protagonist wording stable in the final prompt: introduce `主体1` once, then reuse `主体1` or the same short role label in every beat that contains the person / 最终 prompt 中人物写法要稳定：先定义“主体1”，之后每个有人物的节拍都复用“主体1”或同一个短角色标签。
...
3. Use 小云雀 through `sandbox_generate_image` to match and generate a product-appropriate creator portrait when no suitable person reference is supplied / 没有合适真人参考图时，调用 `sandbox_generate_image` 让小云雀匹配商品并生成达人形象
4. Let the user adjust the creator portrait if needed / 用户可反馈调整达人形象
5. Generate the 30-second sales script from `hook-library.md`, `benefit-voiceover-library.md`, and `cta-library.md` / 基于 `hook-library.md`、`benefit-voiceover-library.md` 和 `cta-library.md` 生成 30 秒带货脚本
6. Generate the final video prompt or proceed to video generation after confirmation / 确认后生成视频 prompt 或直接生成视频
...
After the user confirms parameters and provides any optional materials, first generate or bind the active creator portrait, then build `video_generation_materials`, then generate a short rationale, script, and compact `video_generation_prompt` for user confirmation or as the attached generation prompt content. Keep all readable, directly usable, and Chinese-only by default.

用户确认参数并提供可选素材后，先生成或绑定当前达人形象，再整理 `video_generation_materials`，再生成一个简短思路说明、脚本和简洁的 `video_generation_prompt` 供用户确认，或作为视频生成的附件 prompt 内容。默认只用中文，保持易读且可直接调用。
...
- If the user says `达人不合适`, `换个达人`, `发型改一下`, `穿搭调整`, `更像宝妈`, `更年轻`, `更生活化`, or similar, regenerate the portrait with `sandbox_generate_image`, then refresh the script and prompt / 用户反馈达人形象不合适时，先重新调用 `sandbox_generate_image` 改图，再更新脚本和 prompt
...
- When the user confirms generation, pass `video_generation_materials` together with `video_generation_prompt` to the video generation tool or frontend pipeline.
- The prompt should state that user product materials define product appearance and `creator_portrait_image` defines the protagonist/person identity.
...
- 用户确认生成时，把 `video_generation_materials` 和 `video_generation_prompt` 一起传给视频生成工具或前端生成链路。
- prompt 中说明用户商品素材决定商品外观，`creator_portrait_image` 决定主体人物形象。
...
思路简述：
[用 1-3 句说明为什么选择这个达人形象、钩子、卖点证明和转化路径。]

creator_portrait_image
[已提供或小云雀已生成的达人画像引用。]

video_generation_materials
[用于视频生成的垫图/参考图列表：用户上传商品图/参考图/文件在前，当前 creator_portrait_image 在后。]

script
[0-3s hook / 3-8s 利益点证明 / 8-14s 结果确认 / 14-21s 第二证明 / 21-27s 价值确认 / 27-30s CTA 的简洁脚本，每段包含动作、spoken_line 和可选花字。]
...
When the user replies `我已确认以上信息`, `确认`, `可以`, `符合预期`, `生成视频`, or similar, directly generate the video using the confirmed `video_generation_materials`, `creator_portrait_image`, script, `video_generation_prompt`, and assets. The next user-facing step should be video generation.

当用户回复“我已确认以上信息”“确认”“可以”“符合预期”“生成视频”等表达时，告知用户确认后即可开始生成，并直接基于已确认的 `video_generation_materials`、`creator_portrait_image`、脚本、`video_generation_prompt` 和素材生成视频。下一步面向用户进入视频生成。
...
如果用户认为达人形象、脚本或 `video_generation_prompt` 有问题，直接按用户反馈修改对应部分。达人形象问题先调用 `sandbox_generate_image` 更新图片，再同步调整脚本和 `video_generation_prompt`；脚本或 prompt 问题则直接修改文本。不要重新走参数确认流程，除非修改依赖缺失的核心素材或无法判断的关键决策。修改后仍用同样的格式展示更新后的内容，并询问是否生成视频。
...
video_generation_materials
[如本次改了达人形象，使用更新后的垫图列表；如未改，使用当前确认版本。]
...
思路简述：
[briefly explain creator fit, hook, proof, and CTA logic]

creator_portrait_image
[supplied or 小云雀-generated creator portrait reference]

video_generation_materials
[user uploaded materials first, active creator portrait last]

script
[concise 30-second script]
...
确认达人形象和脚本后即可开始生成视频。默认按 9:16 竖版抖音带货视频产出。如需调整，可修改达人形象、尺寸、平台或补充风格/节奏/卖点/模特等要求。
```

Do not output the full reasoning, category strategy, creative strategy, or long planning document by default. Use a short rationale plus `creator_portrait_image`, a concise `script`, and `video_generation_prompt` as the default confirmation and generation artifact. Only include analysis, multiple options, or long shooting plans when the user explicitly asks for them.
