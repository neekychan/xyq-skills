...
用法：
  python3 validate_storyboard.py <storyboard.md> --max-duration L [--target N] [--mode hard|soft|budget]
...
PROFILES = {
    15: {"img": 9, "vid": 3, "aud": 3, "mixed": 12, "audio_only": False},
    30: {"img": 30, "vid": 10, "aud": 10, "mixed": 20, "audio_only": True},
}
...
def main():
    ...
    L = args.max_duration
    prof = PROFILES[15] if L <= 15 else PROFILES[30]
    tier = "15 档" if L <= 15 else "30 档"
    ...
    def assets():
        for s in shots:
            ...
            if ni > prof["img"]:
                add("ERROR", f"shot_{n:02d}: 图片 {ni} 张 > {tier}上限 {prof['img']}", fix="删减素材到档位上限内")
            if nv > prof["vid"]:
                add("ERROR", f"shot_{n:02d}: 参考视频 {nv} 个 > {tier}上限 {prof['vid']}", fix="删减素材到档位上限内")
            if na > prof["aud"]:
                add("ERROR", f"shot_{n:02d}: 参考音频 {na} 段 > {tier}上限 {prof['aud']}", fix="删减素材到档位上限内")
            if ni + nv > prof["mixed"]:
                add("ERROR", f"shot_{n:02d}: 图片+视频合计 {ni+nv} > {tier}上限 {prof['mixed']}", fix="删减素材到档位上限内")
            if na and not ni and not prof["audio_only"]:
                add("ERROR", f"shot_{n:02d}: {tier}禁止只传 AudioList 不传图片", fix="至少传入角色或场景参考图")
            for kind, cnt, tag in (("图片", ni, "图片"), ("视频", nv, "视频"), ("音频", na, "音频")):
                refs = [int(x) for x in re.findall(rf"@{tag}(\d+)", s["prompt"])]
                if refs and max(refs) > cnt:
                    add("ERROR", f"shot_{n:02d}: Prompt 引用 @{tag}{max(refs)} 但 {kind}列表只有 {cnt} 项", fix="对齐引用与列表")
    ...
    # 可合并段（贪心：相邻段时长之和 ≤ 上限 且 素材并集不超档 → 提示合并，减少生成调用）
    def mergeable():
        ...
        def fits(window):
            ...
            return (len(imgs) <= prof["img"] and len(vids) <= prof["vid"]
                    and len(auds) <= prof["aud"] and len(imgs) + len(vids) <= prof["mixed"])
        ...
        while i < len(seq):
            ...
            if len(window) >= 2:
                ...
                add("WARN", f"{span}: {len(window)} 段合计 {total:g}s ≤ 单段上限 {L:g}s 且素材并集不超档", fix="疑似可合并为一次生成调用，除非密度门判定超载才保留拆分")
    ...
    print(f"档位：{tier}（Duration 4~{L:g}s，图≤{prof['img']}，视频≤{prof['vid']}，音频≤{prof['aud']}，图+视频≤{prof['mixed']}，仅音频{'允许' if prof['audio_only'] else '禁止'}）")
