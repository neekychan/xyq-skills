---
name: brand-film
description: Create modern TVC-style brand story or brand film concepts by turning category tension, audience insight, brand assets, cultural timing, and product role into a cinematic, shootable film route. Use when the user asks for 品牌大片, brand story, brand film, TVC, 品牌TVC, campaign film, hero film, 品牌故事片, 品牌感视频, 现代广告片, category-based brand-film thinking, or wants to build a reusable framework for high-quality brand videos.
---
...
Reference files are addressed as `/workspace/.skills/brand-film/references/xxx.md`; when a reference is needed, read it with the `sandbox_read` tool.
...
- `/workspace/.skills/brand-film/references/tvc-type-routing.md`
- `/workspace/.skills/brand-film/references/style-reference-routing.md`
- `/workspace/.skills/brand-film/references/compact-spec-output.md` unless the user explicitly asks for a fast prompt, chat-only answer, no file creation, or strategy-only guidance.
- `/workspace/.skills/brand-film/references/seedance2-brand-prompt.md` when writing a final video prompt.

Read only the additional references that the task triggers:
- category reference when the product/category is concrete.
- `/workspace/.skills/brand-film/references/current-trend-signals.md` when expression form, trend fit, AI/CGI, mixed-media, creator-style realism, or modular campaign judgment matters.
- `/workspace/.skills/brand-film/references/vfx-expression-routing.md` when using VFX, CGI, motion graphics, animation, mixed media, virtual production, surreal transformation, impossible scale, product mechanism visualization, or generated-video spectacle.
...
3. Read `/workspace/.skills/brand-film/references/current-trend-signals.md` for expression-form judgment.
4. Read `/workspace/.skills/brand-film/references/tvc-type-routing.md` unless the user already specified the exact commercial type.
5. Read `/workspace/.skills/brand-film/references/vfx-expression-routing.md` when the route may use CGI/VFX, motion graphics, animation, mixed media, virtual production, surreal transformation, product mechanism visualization, impossible scale, or generated-video spectacle.
6. Read `/workspace/.skills/brand-film/references/style-reference-routing.md` before choosing the visual style.
7. Read `/workspace/.skills/brand-film/references/seedance2-brand-prompt.md` before writing a final Seedance 2.0 prompt.
8. Read `/workspace/.skills/brand-film/references/compact-spec-output.md` when creating the default standalone creative plan file.
9. Read specialized craft references only when needed:
   - `/workspace/.skills/brand-film/references/shot-logic-and-continuity.md` for filmable evidence, narrative beats, and motion spine.
   - `/workspace/.skills/brand-film/references/lighting-design.md` for lighting rules and shot-level lighting language.
   - `/workspace/.skills/brand-film/references/sound-and-music.md` for BGM density, sound motif, silence, and sonic lockup.
   - `/workspace/.skills/brand-film/references/originality-checks.md` for anti-generic checks and failure modes.
...
- When output is likely to become generated video, include a paste-ready 15-second Seedance 2.0 prompt even if the user did not explicitly say "prompt".
- Default to `时长：15秒以内，比例：16:9。` for brand films, TVCs, campaign hero films, luxury/fashion/fragrance image films, and cinematic routes. Use `9:16` only when the user asks for vertical, mobile-first, TikTok/Reels/Shorts, social-first, or phone-native delivery.
- Use 3-4 beats for 15-second brand films unless the user changes the duration.
...
- **Sport** -> `/workspace/.skills/brand-film/references/categories/sport.md`
- **Beauty / personal care** -> `/workspace/.skills/brand-film/references/categories/beauty-personal-care.md`
- **Consumer tech / AI** -> `/workspace/.skills/brand-film/references/categories/consumer-tech-ai.md`
- **Auto / mobility** -> `/workspace/.skills/brand-film/references/categories/auto-mobility.md`
- **Fashion / lifestyle** -> `/workspace/.skills/brand-film/references/categories/fashion-lifestyle.md`
- **Food / beverage / alcohol** -> `/workspace/.skills/brand-film/references/categories/food-beverage-alcohol.md`
- **Service / platform / travel / marketplace** -> `/workspace/.skills/brand-film/references/categories/service-platform-travel.md`
- **B2B / productivity** -> `/workspace/.skills/brand-film/references/categories/b2b-productivity.md`
- **Finance / insurance / health-adjacent** -> `/workspace/.skills/brand-film/references/categories/finance-insurance-health.md`
...
Read `/workspace/.skills/brand-film/references/tvc-type-routing.md` to choose the commercial type / production approach. The selected type must define visual style, lighting, camera/lens, editing rhythm, sound design, main subject, scene design, story structure, 15s beat template, product/brand role, and prompt requirements. Do not use type labels as decoration.

Read `/workspace/.skills/brand-film/references/style-reference-routing.md` to choose a reference direction by brand task, not by personal taste. For Seedance 2.0, the style reference must be a strong film / advertising style anchor, not a mild mood label. Translate it into camera language, visual/art direction, color/material system, lighting behavior, editing rhythm, and sound motif.

When using surreal, uncanny, dreamlike, impossible, mythic transformation, heightened visual logic, or any phrase like `超现实`, first define one visible world rule through `/workspace/.skills/brand-film/references/style-reference-routing.md` before writing the final prompt.
...
When the user asks for classic cases, benchmarks, brand-film examples, or wants a concept that references Dior, Nike, On Running, Apple, or similar canonical brands, read `/workspace/.skills/brand-film/references/classic-cases.md`. Use the cases as creative mechanisms, not as templates to copy.
...
Use `dynamic_questionnaire` for user decisions that affect whether the next turn should generate video.

Creative direction gate:

- If multiple viable brand-film routes, campaign routes, style directions, or concepts are shown to the user for choice, do not continue with a pure-text selection prompt.
- First show a compact comparison of the options, then call `dynamic_questionnaire` with one `radio` question asking the user to choose the creative direction.
- Each option should be one concise label plus a short description. Include an additional option such as `我想先调整方向` when revision is reasonable.
- Do not write the final complete creative plan markdown or final `video_generation_prompt` for one route until the user has selected a direction, unless the user explicitly asked to receive multiple complete prompts without choosing.

Video generation confirmation gate:

- After the final creative plan and paste-ready `video_generation_prompt` are visible to the user, do not end with only a natural-language sentence such as `你说同意生成`.
- Call `dynamic_questionnaire` with one `radio` question:
  - `继续生成视频`: proceed to the video-generation tool or frontend generation handoff using the displayed plan and prompt.
  - `先修改方案`: wait for the user's revision request and do not generate video yet.
- The confirmation applies only to the displayed version. If the user changes the script, creative plan, prompt, assets, duration, aspect ratio, style, or model, update the visible package and ask the same confirmation again before generation.
- Do not call `sandbox_generate_video` or any equivalent generation handoff before this confirmation is answered with `继续生成视频`.
...
**Compact creative-plan rule**: for concrete brand-film, TVC, campaign film, brand-story, product-promotion, storyboard-prompt, or Seedance/video-generation tasks, read `/workspace/.skills/brand-film/references/compact-spec-output.md` and create a readable standalone complete creative plan markdown file by default, unless the user explicitly asks for a fast prompt, chat-only answer, no file creation, or strategy-only guidance. Before the final chat response, write the complete creative plan with `sandbox_write` to the designated output path defined in `compact-spec-output.md`. Follow the file naming rules from `compact-spec-output.md`; do not define a separate naming scheme in `SKILL.md`. Do not merely mention a path in chat without writing the file, and do not place output files inside `/workspace/.skills/brand-film/`. Do not weaken the final Seedance 2.0 prompt to make the chat shorter. Move route reasoning, category diagnosis, trend judgment, originality checks, and risks into the complete creative plan file. In the chat, use the user-facing structure `大的思路总结` / `完整创意方案可见：{written_output_path}` / `这是为你规划好的 prompt`, then call the Video generation confirmation gate above.
...
Only when the user explicitly asks for multiple routes, output 3-5 distinct routes. Each route must include a compact big-idea summary and a `video_generation_prompt` for Seedance 2.0. Do not make the user select a route before seeing usable prompts. If the next step requires choosing one route to create the final plan, final prompt, or video generation, use the Creative direction gate above.

## Seedance 2.0 Prompt Rules

Read `/workspace/.skills/brand-film/references/seedance2-brand-prompt.md` before writing final prompts. Default to Seedance 2.0 and 15-second videos unless the user asks for another model, duration, or delivery format.
...
- Start Chinese prompts with `时长：15秒以内，比例：16:9。` by default.
- State the task type: generate, reference, edit, extend, or stitch.
- Use 3-4 timed beats for 15 seconds.
...
- Is there a motion spine across the 15 seconds?
...
- If multiple directions were shown for choice, was `dynamic_questionnaire` used before finalizing one?
- If the final output contains a paste-ready `video_generation_prompt`, was `dynamic_questionnaire` used to ask `继续生成视频` vs `先修改方案`?
