...
- Default visible output is `creator_portrait_image` plus a concise timestamped `script`. Compile the full `seedance_generation_package` internally for the video tool; show it only when explicitly requested / 默认用户可见输出为 `creator_portrait_image` 和简洁时间轴 `script`。完整 `seedance_generation_package` 在内部编译并传给视频工具，只有用户明确要求时才展示
...
6. Compile the result and all applicable reference rules into one self-contained `seedance_generation_package` / 将结果和所有适用 reference 规则编译成一个自包含的 `seedance_generation_package`
...
2. `3-7s` keep the next logical script point and show product/context / 保留下一句逻辑重点，并展示产品或场景
3. `7-10s` turn the main claim into product action or visible proof / 把主卖点句转成产品动作或可见证明
4. `10-13s` compress supporting copy into one short line plus result/reaction / 把辅助文案压缩成一句短口播，加结果或反应
5. `13-15s` preserve CTA, product in frame, and lip-sync / 保留 CTA，产品在画面中，口型同步
...
5. Show the timestamped script to the user and compile the detailed tool package internally / 向用户展示时间轴脚本，并在内部编译详细工具包
...
2. `3-7s` adapt the reference context and creator energy to the user's product scene / 把参考视频的场景推进和达人状态迁移到用户商品场景
3. `7-10s` adapt the proof mechanism with user's product action, result, or comparison / 用用户商品动作、结果或对比复刻证明机制
4. `10-13s` adapt the reveal/payoff with visible proof and expression change / 用可见证明和表情变化复刻揭示/爽点
5. `13-15s` adapt CTA shape for the target platform with new wording / 按目标平台复刻 CTA 形态，但文案重写
...
5. Show the safer adapted script and compile its detailed generation package internally / 展示更稳妥的改写脚本，并在内部编译详细生成包
