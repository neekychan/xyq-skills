...
- Before creating the spec file, if there are multiple viable creative directions, story routes, short-drama genre containers, selling-angle routes, or remix candidates, call `dynamic_questionnaire` to let the user choose. Stop after the questionnaire and do not write the spec file or final `video_generation_prompt` until the user selects a direction.
...
- In chat, show only the route summary, the spec markdown link, the final `video_generation_prompt`, and the next-step video generation confirmation.
...
- If the user asks for multiple routes as the final deliverable, show a compact route table in chat and put full route specs in the markdown. If the multiple routes are candidate directions before a final plan or prompt, use `dynamic_questionnaire` instead of a plain text route table.
...
下一步我可以继续生成视频，或先按你的意见修改方案。
```

If `dynamic_questionnaire` is available after the spec and prompt are ready, ask the next-step question with two options: `继续生成视频` and `先修改方案`. Do not call video generation tools until the user chooses `继续生成视频`.
