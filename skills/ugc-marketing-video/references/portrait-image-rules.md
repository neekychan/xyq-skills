...
The creator portrait is a character reference for the downstream script and video generation. It should help the video protagonist feel real, product-matched, and stable, but it must not redefine the product or force unwanted actions into the video. In this skill, `sandbox_generate_image` uses Nano Banana Pro for creator portrait generation.

## Required Tool

Creator portrait generation must use Nano Banana Pro through:
...
Before calling `sandbox_generate_image`, internally build a complete high-quality Nano Banana Pro creator image prompt. Keep the model implementation internal and do not expose the full prompt.
