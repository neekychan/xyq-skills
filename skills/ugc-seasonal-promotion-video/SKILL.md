...
# UGC Seasonal Promotion Video 15s
...
- If multiple viable campaign routes, promotion hooks, story hooks, creator directions, or offer expressions are shown to the user for choice, do not continue with a pure-text selection prompt.
- First show a compact comparison of the options, then call `dynamic_questionnaire` with one `radio` question asking the user to choose the direction.
...
- Do not write the final `video_generation_prompt` for one route until the user has selected a direction, unless the user explicitly asked to receive multiple complete prompts without choosing.

Video generation confirmation gate:

- After the final script and paste-ready `video_generation_prompt` are visible to the user, do not end with only a natural-language consent sentence.
- Call `dynamic_questionnaire` with one `radio` question:
  - `继续生成视频`: proceed to the video-generation tool or frontend generation handoff using the displayed prompt.
  - `先修改方案`: wait for the user's revision request and do not generate video yet.
- The confirmation applies only to the displayed version. If the user changes the script, prompt, assets, campaign offer, CTA, duration, aspect ratio, subtitles, style, or model, update the visible package and ask the same confirmation again before generation.
- Do not call `sandbox_generate_video` or any equivalent generation handoff before this confirmation is answered with `继续生成视频`.

## Duration Support

- This package generates 15-second campaign promotion videos only.
- 15s uses the compact 5-beat spine: `Hook -> brightest offer/mechanism -> one product proof sentence -> campaign time + how to get the offer -> purchase CTA`.
- Keep promotion value dominant and compact. Do not expand into long product education.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/full-campaign-promotion-rules.md` before deciding the promotion structure.
2. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/product-image-and-script-lock.md` before using product images, product screenshots, fixed scripts, product cards, package photos, or user-supplied copy.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/seedance-positive-execution.md` before writing the final prompt.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/campaign-occasion-routing.md` before choosing campaign wording, motive, visual symbols, and CTA.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/promotion-hook-and-benefit-rhythm.md` before choosing the hook and benefit order.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/screenshot-phone-ui-rules.md` before describing screenshot, phone, or card presentation.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/creator-persona-routing.md` before defining persona, styling, expression, role fit, and creator timing.
...
1. Read `/workspace/.skills/ugc-seasonal-promotion-video/references/qa-checklist.md` before answering.

Before any final deliverable, silently run the QA checklist logic from `/workspace/.skills/ugc-seasonal-promotion-video/references/qa-checklist.md` when the output is a complete script, storyboard, or video prompt.
...
- Set duration to 15s.
- Use this 15s script structure by default: `Hook -> brightest offer/mechanism -> one product function/proof sentence -> campaign time + how to get the offer -> purchase CTA`.
...
- Product proof is compact. Use one product proof beat, such as texture, capacity, taste, function, try-on, parameter, or use scenario.
...
- Short stickers are selective foreground emphasis layers. Select 2-3 sticker moments for the 15s video: Hook, brightest offer, campaign path, or CTA. Place stickers in clean foreground safe zones with clear margins, above the visual layer, beside the product or in upper/lower empty space. Keep product, hands, face, logo, package text, and gift pile readable.
...
Default output is one paste-ready `video_generation_prompt` for Seedance 2.0 / Pippit / Xiaoyunque. After showing the prompt, call the Video generation confirmation gate above.
...
花字策略：
[15秒选择2-3个花字节点；花字作为前景强调层，放在画面空白安全区，与商品主体、手、脸、logo、包装文字和赠品堆叠区并列排布；产品证明段使用清爽画面承载动作信息]
...
- 画面动作：
- 镜头：
- 中文口播：[Hook句，可以是反差、夸张反应、福利奇观、0.01元/赠品/价格震惊，后续用不同句式推进]
- 短花字：[从全片2-3个花字节点中选择；如使用，放在前景安全区，与商品/手/脸并列清晰呈现]
- 声画匹配：
...
- 画面动作：
- 镜头：
- 中文口播：[最亮利益点句，直接说价格/机制/赠品/到手量中最强的一项]
- 短花字：[从全片2-3个花字节点中选择；如使用，放在前景安全区，与商品/手/脸并列清晰呈现]
- 声画匹配：

5-8s / [中文镜头标题]
- 本镜任务：一句话突出或展现产品功效/功能/规格/使用场景
- 人物呈现：[有人物则写妆发服饰动作；无人物则写“无人物出镜，产品/赠品/机制卡为主体”]
- 画面动作：
- 镜头：
- 中文口播：[产品证明句，一句话说明为什么值得买，例如功效、功能、规格、肤感、口味、容量、使用场景]
...
- 画面动作：
- 镜头：
- 中文口播：[活动时间+领券/进直播间/点商品卡/小黄车/下单路径句]
- 短花字：[从全片2-3个花字节点中选择；如使用，放在前景安全区，与商品/手/脸并列清晰呈现]
- 声画匹配：
