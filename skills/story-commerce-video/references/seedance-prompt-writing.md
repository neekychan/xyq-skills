...
20260716 base update: for product reference images, constrain the prompt to reference-visible product surfaces only, avoid invented interiors/back views/contents, prefer low-risk no-tear product display when package opening is fragile, require AB comparison to use a separate comparison product rather than two copies of the marketed product, and use only selling points provided by the user or visible/source-backed product information.
...
- Define selling-point anchors only from user-provided text, visible package text, product title/listing text, or other explicit source material. If the selling point is not source-backed, do not write it as a product fact.
...
卖点真实性约束：只使用用户明确提供、商品标题/详情明确提供、或图片1正面可见文字能支持的卖点；不得从品类常识自动脑补配料、功效、认证、规格、口味、容量、产地、奖项、背标信息或内部内容。脚本必须适配真实已知卖点；如果某个卖点缺少可视化依据，用人物口播、场景结果、使用感受或见证者反应表达，不生成虚构包装文字、配料表、背面标签、剖面图或内容物。
...
- Are all selling points source-backed by user text, product title/listing details, visible front-package text, or other provided materials?
