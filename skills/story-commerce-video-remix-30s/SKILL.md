---
name: story-commerce-video-remix-30s
description: >-
  Create narrative commerce short-video concepts focused on 狗血/霸总/绿茶/小三/宫斗/反转/重生 and creative micro-drama containers such as 古装错位、恋综、审判、末日、科幻、时间循环. After picking one genre direction, outputs a new-model-ready 30-second video_generation_prompt with 5-6 timestamped shots, global visual direction, distinct subject character design, product-name dialogue exposure, and director-style beat fields ordered as 拍法/画面内容/商品露出/声音, with 商品露出 omitted whenever the product is not visible in that beat. Also supports calling pippit_omni_search with type marketing_hotpoint to search hotspot creative content from the user's original request and user images, then runs hotspot SP/product matching, hot-video core-play remix, and remixed-SP scoring when suitable content is returned. Use when the user wants 短剧风创意带货, 剧情向带货短视频, 30秒广告脚本, 短剧爽感, 三秒hook, 爆款开头, 商品推广创意, 带货脚本, 分镜提示词, Seedance提示词, 热点remix, 热点融合, SP评估, product-led story ads, or a reusable thinking framework that maps audience, emotion, conflict, camera, pacing, sound, and product proof into a shootable conversion video.
---
...
1. Read `/workspace/.skills/story-commerce-video-remix-30s/references/genre-routing.md`（需要时使用 sandbox_read 工具读取） before choosing the route.
...
1. Read `/workspace/.skills/story-commerce-video-remix-30s/references/hotspot-remix-evaluation.md`（需要时使用 sandbox_read 工具读取） before rewriting, scoring, or deciding whether the hotspot can carry the product.
...
1. Read `/workspace/.skills/story-commerce-video-remix-30s/references/seedance-prompt-writing.md`（需要时使用 sandbox_read 工具读取） before writing the final prompt.
...
1. Read `/workspace/.skills/story-commerce-video-remix-30s/references/compact-spec-output.md`（需要时使用 sandbox_read 工具读取） before writing the plan file.
...
1. Read `/workspace/.skills/story-commerce-video-remix-30s/references/critique-checklist.md`（需要时使用 sandbox_read 工具读取） before answering.
...
- If a Chinese `video_generation_prompt` is included, it starts with `时长：30秒，比例：9:16。`
- The prompt contains 5-6 timestamped shots by default for 30-second output.
...
- Hotspot remix outputs preserve the original script's pure-text narrative format, include timeline details for multiple shots, keep total duration within 25-30 seconds, explicitly show the product/product image, and mention the product name.
...
- If multiple viable creative directions, routes, genres, hotspot candidates, or remixed concepts are shown to the user for choice, do not continue with a pure-text selection prompt.
...
- Do not write the final plan markdown or final `video_generation_prompt` for one route until the user has selected a direction, unless the user explicitly asked to receive multiple complete prompts without choosing.
...
For multiple routes, output 3-6 distinct concepts. Vary the short-drama genre, creative container, audience, visual reference, and product plot function rather than writing small wording variations of the same idea. If the next step requires choosing one route to create the final plan, final prompt, or video generation, use the Creative direction gate above.

## Seedance 2.0 Prompt Writing

Default deliverable is one pasteable 30-second `video_generation_prompt` after one genre direction is selected. Whenever this skill produces a final `video_generation_prompt`, the Seedance reference is mandatory, regardless of whether the user explicitly mentions Seedance.
...
- Start Chinese prompts with `时长：30秒，比例：9:16。`
...
- For 30s prompts, output 5-6 timestamped shots by default. Use this pacing: `0-3秒` abnormal hook + relationship pressure, `3-8秒` pressure escalation and stakes lock, `8-14秒` protagonist hidden choice + product/ability activation, `14-22秒` main visible proof escalation, `22-27秒` genre payoff and witness reaction, `27-30秒` product-name conversion line and product/image high-light close-up. If the story is simple, combine payoff and conversion into a 5-shot structure; if the product proof needs more clarity, keep all 6 shots.
...
- If the final output contains a paste-ready `video_generation_prompt`, use `dynamic_questionnaire` to ask `继续生成视频` vs `先修改方案` after the prompt is visible.
...
- **Physical consumer goods / home goods / cleaning**: `/workspace/.skills/story-commerce-video-remix-30s/references/physical-goods.md`（需要时使用 sandbox_read 工具读取）
- **Food and beverage**: `/workspace/.skills/story-commerce-video-remix-30s/references/food-beverage.md`（需要时使用 sandbox_read 工具读取）
- **Beauty and personal care**: `/workspace/.skills/story-commerce-video-remix-30s/references/beauty-personal-care.md`（需要时使用 sandbox_read 工具读取）
- **Fashion / shoes / accessories / lifestyle**: `/workspace/.skills/story-commerce-video-remix-30s/references/fashion-lifestyle.md`（需要时使用 sandbox_read 工具读取）
- **Services / courses / local business**: `/workspace/.skills/story-commerce-video-remix-30s/references/services-courses.md`（需要时使用 sandbox_read 工具读取）
- **Apps / software / AI tools**: `/workspace/.skills/story-commerce-video-remix-30s/references/software-ai-tools.md`（需要时使用 sandbox_read 工具读取）
- **B2B / enterprise / high-consideration products**: `/workspace/.skills/story-commerce-video-remix-30s/references/b2b-enterprise.md`（需要时使用 sandbox_read 工具读取）
