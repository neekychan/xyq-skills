...
Use this reference when a brand-film response would otherwise become long, mixed-language, or overloaded with strategy notes. The goal is to create a good conversation experience while preserving a full-quality Seedance 2.0 `Video Prompt`.
...
3. **Seedance 2.0 Video Prompt**: paste-ready generation prompt, never weakened by the compact chat format.
...
- After showing the final planned prompt, call `dynamic_questionnaire` once to ask whether to continue video generation or revise the plan. Do not rely on a plain-text consent sentence.
...
- If the user asks for multiple routes, show a compact route table in chat and put full route specs in the markdown. Each selected route still needs a usable Seedance 2.0 prompt unless the user asks for strategy only.
...
Then call `dynamic_questionnaire` with one radio question:

- `继续生成视频`: continue with video generation from the displayed plan and prompt.
...
- `时长：15秒以内，比例：16:9。` by default for cinematic brand films, unless the user asks for vertical/social-first.
...
- 3-4 timestamped beats for 15-second films unless the user changes duration.
